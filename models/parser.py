...
class Parser(nn.Module):
	...
	def forward(self, words, postags, chars, heads, labels, masks):
		...
		rnn1_out, rnn2_out, uni_fw, uni_bw = self.encoder(words, postags, chars)
	...
	def predict_batch(self, words, postags, chars, lengths):
		rnn1_out, rnn2_out, uni_fw, uni_bw = self.encoder(words, postags, chars)
		head_repr, dep_repr = self.get_encoder_repr(rnn1_out, rnn2_out, uni_fw, uni_bw)
		arc_score, lab_score = self.scorer(head_repr, dep_repr)
		return self.parse_from_score(arc_score, lab_score, lengths)

	def predict_batch_with_loss(self, words, postags, chars, heads, labels, pad_masks, lengths):
		rnn1_out, rnn2_out, uni_fw, uni_bw = self.encoder(words, postags, chars)
		head_repr, dep_repr = self.get_encoder_repr(rnn1_out, rnn2_out, uni_fw, uni_bw)
		arc_score, lab_score = self.scorer(head_repr, dep_repr)
