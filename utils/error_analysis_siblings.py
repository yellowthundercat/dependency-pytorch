...
# print(sentence_lst)
# {'index': [0, '1', '2'], 'index': ['<root>', 'Cảnh_báo', '!'], 'pos': ['ROOT', 'v', '.'], 'system_head': [0, '0', '1'], 'gold_head': [0, '0', '1'], 'system_label': ['0', 'root', 'punct'], 'gold_label': ['0', 'root', 'punct']}

gold = {} # {'x': '# indexs having x siblings on GOLD graph'}
pred = {} # {'x': '# indexs having x siblings on PREDICT graph'}
correct_precision = {}
correct_recall = {}

for sen in sentence_lst:
  gold_dependents = {} # 'index': '#dependents of index'
  pred_dependents = {}
  correct_gold_dependents = {}
  correct_pred_dependents = {}
  gold_siblings = {} # 'index': '#siblings of index'
  pred_siblings = {}
  ...
  for i in range(0, len(sen['index'])):
    ...
    correct_gold_dependents[sen['index'][i]] = 0
    correct_pred_dependents[sen['index'][i]] = 0
    gold_siblings[sen['index'][i]] = 0
    pred_siblings[sen['index'][i]] = 0
    correct_gold_siblings[sen['index'][i]] = 0
    correct_pred_siblings[sen['index'][i]] = 0

  for i in range(1, len(sen['index'])):
    # for each index i, find its head and count number of dependents of the head on gold & predict graph
    ...
    # for each index, count number of correct dependents for gold & predict graph
    if gold_head_idx == system_head_idx and gold_label_idx == system_label_idx:
      correct_gold_dependents[sen['index'][gold_head_idx]] += 1
      correct_pred_dependents[sen['index'][system_head_idx]] += 1
  ...
  for i in range(1, len(sen['index'])):
    ...
    gold_siblings[sen['index'][i]] = max(0, gold_dependents[sen['index'][gold_head_idx]] - 1)
    pred_siblings[sen['index'][i]] = max(0, pred_dependents[sen['index'][system_head_idx]] - 1)

    correct_gold_siblings[sen['index'][i]] = max(0, correct_gold_dependents[sen['index'][gold_head_idx]] - 1)
    correct_pred_siblings[sen['index'][i]] = max(0, correct_pred_dependents[sen['index'][system_head_idx]] - 1)
