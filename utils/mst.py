...
def mst(scores, eps=1e-10):
    ...
    """
    # moving score to positive
    min_value = np.amin(scores)
    if min_value <= 0:
        scores += (abs(min_value) + 1)
    scores = scores.T
...
if __name__ == '__main__':
    scores = np.array([[-0.01892652, -0.02148798, -0.02304527, -0.0242483],
                       [-0.0343194, -0.03883664, -0.0416051, -0.0437656],
                       [-0.04960036, -0.0560579, -0.06000825, -0.06307747],
                       [-0.07191796, -0.08120494, -0.08685522, -0.09119]])
    print(mst(scores))